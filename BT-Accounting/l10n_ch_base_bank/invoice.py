...
class AccountInvoice(Model):
    ...
    def onchange_partner_id(self, cursor, uid, ids, invoice_type, partner_id,
        ...
        if partner_id:
            if invoice_type in ('in_invoice', 'in_refund'):
                ...
                bank_id = partner_bank_id # HACK: 02.07.2014 16:59:37: olivier: set partner_bank_id as bank_id, otherwise it will compare these two further down and then it would do an onchange_partner_bank which is not right
                user = self.pool.get('res.users').browse(cursor, uid, uid, context)
                bank_ids = user.company_id.partner_id.bank_ids
                if bank_ids and not partner_bank_id: # HACK: 02.07.2014 17:00:17: olivier: just set new bank if it was empty (otherwise it overwrites a default value that could be set for this field)
                    bank_id = bank_ids[0].id # HACK: 02.07.2014 17:02:32: olivier: if new bank_id can be set, set it as well as "bank_id"
                    res['value']['partner_bank_id'] = bank_id
        ...
        for invoice in self.browse(cursor, user, ids):
            if invoice.type in 'in_invoice':
                if invoice.partner_bank_id and invoice.partner_bank_id.state == 'bvr' and \
                    ...
                    raise_message += _("Invoice '%s'") % (invoice.number,) + "\n"
        if raise_message != '':
            raise osv.except_osv(_('Error: BVR reference is required'), raise_message)
        return True
    ...
    def _check_bvr(self, cr, uid, ids, context=None):
        ...
        for invoice in invoices:
            #begin hack piti1: allow canceling draft invoices
            #if invoice.reference_type == 'bvr' and invoice.state != 'draft':
            if invoice.reference_type == 'bvr' and invoice.state not in  ('draft','cancel'):
            # end hack piti1
