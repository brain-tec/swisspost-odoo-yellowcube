            ...
            for val in name.split(' '):
                for col in cols:
                    ...
                    if tmp_ids:
                        ids += tmp_ids
                        break
        # HACK: 24.06.2014 15:00:07: olivier: if nothing is typed in the search, then return the super name_search
        else:
            return super(Bank, self).name_search(cursor, uid, name, args=args, operator=operator, context=context, limit=limit)
