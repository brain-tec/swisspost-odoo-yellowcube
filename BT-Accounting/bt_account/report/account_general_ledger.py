...
class general_ledger(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if (data['model'] == 'ir.ui.menu'):
            ...
            if self.account_id:
                #hack jool 6.1: get account_id[0]
#                objects = self.pool.get('account.account').browse(self.cr, self.uid, [self.account_id])
                objects = self.pool.get('account.account').browse(self.cr, self.uid, [self.account_id[0]], context=ctx)
            else:
                objects = self.pool.get('account.account').browse(self.cr, self.uid, new_ids, context=ctx)
    ...
    def get_children_accounts(self, account, get_only_first_account=False):
        res = []
    ...
    def lines(self, account):
        ...
        if res_lines and self.init_balance:
            ...
            print 'self.init_query: ', self.init_query
            sql = """
                SELECT 0 AS lid, '' AS ldate, '' AS lcode, COALESCE(SUM(l.amount_currency),0.0) AS amount_currency, 'Initial Balance' AS lref, '' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, '' AS lperiod_id, '' AS lpartner_id,
        ...
        account_sum = 0.0
        for l in res:
            if l['lref'] == 'Initial Balance':
                currency = account.currency_id and account.currency_id
                if currency:
                    l['currency_id'] = currency.id
                    l['currency_code'] = currency.symbol
            ...
            if l['credit'] - l['debit']> 0:
                if l['amount_currency'] <> 0 and l['amount_currency'] <> None:
                    l['amount_currency'] = abs(l['amount_currency']) * -1
            if l['amount_currency']  <> 0 and l['amount_currency'] <> None:
                self.tot_currency = self.tot_currency + l['amount_currency']
                #hack jool: set tot_currency as progress_currency
...
report_sxw.report_sxw(
    'report.account.ledger_bt',
    'account.account',
    'addons/bt_account/report/account_general_ledger.rml', 
    parser=general_ledger, 
    header='internal'
)

report_sxw.report_sxw(
    'report.account.ledger_bt_landscape',
    'account.account',
    'addons/bt_account/report/account_general_ledger_landscape.rml', 
    parser=general_ledger, 
    header='internal landscape'
)

#report_sxw.report_sxw(
#    'report.account.general.ledger_landscape', 
#    'account.account', 
