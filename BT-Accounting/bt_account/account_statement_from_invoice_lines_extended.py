...
class account_statement_from_invoice_lines_extended(osv.osv_memory):
    _inherit = 'account.statement.from.invoice.lines'
    
    def populate_statement(self, cr, uid, ids, context=None):
        print 'populate_statement bt_account/account_statement_from_invoice_lines_extended.py'
        if context is None:
            context = {}
        ...
        #hack jool: take statement date if set
#        line_date = time.strftime('%Y-%m-%d')
        line_date = statement.date or time.strftime('%Y-%m-%d')
        ...
        for line in line_obj.browse(cr, uid, line_ids, context=context):
            ...
            #hack jool: set partner_id in context
            context.update({'partner_id': line.partner_id.id})
            type = 'general'
            ttype = amount < 0 and 'payment' or 'receipt'
            #hack jool: found here https://bugs.launchpad.net/openobject-addons/+bug/1257337 -> this was not correct, so I removed it again
...
#                ttype = 'payment'
#                sign = -1
            if line.journal_id.type in ('purchase', 'purchase_refund'):
                type = 'supplier'
            elif line.journal_id.type in ('sale', 'sale_refund'):
                type = 'customer'
            if line.journal_id.type in ('sale', 'purchase_refund'):
                ttype = 'receipt'
            elif line.journal_id.type in ('purchase', 'sale_refund'):
                ttype = 'payment'
                sign = -1
...
#                'date': time.strftime('%Y-%m-%d'),
        ...
        return {'type': 'ir.actions.act_window_close'}
        return super(account_statement_from_invoice_lines_extended, self).populate_statement(self, cr, uid, ids, context=None)
        
account_statement_from_invoice_lines_extended()
