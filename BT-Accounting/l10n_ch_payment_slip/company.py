...
class ResCompany(Model):
    ...
    _columns = {
        ...
        'property_bvr_skonto_debitor_account': fields.property(
            'account.account',
            type='many2one',
            relation='account.account',
            string="BVR Skonto Debitor Account",
            method=True,
            view_load=True,
            required=True,
            domain="[('type', '=', 'other')]",
            help="This Account is used for transferring bvr skonto for debitors."),

        'property_bvr_skonto_creditor_account': fields.property(
            'account.account',
            type='many2one',
            relation='account.account',
            string="BVR Skonto Creditor Account",
            method=True,
            view_load=True,
            required=True,
            domain="[('type', '=', 'other')]",
            help="This Account is used for transferring bvr skonto for creditors."),

        'property_bvr_delay': fields.integer('Delay in which skonto will still be considered.', required=True),
    }
