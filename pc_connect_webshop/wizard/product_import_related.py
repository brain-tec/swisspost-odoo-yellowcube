...
def product_import_related(pool, cr, uid, csv, context):
    if context is None:
        context = {}
    ...
    for row in csv:
        ...
        if default_code != '':
            ...
            product_id = product_pool.search(cr, uid, [('default_code', '=', current_default_code)], context=context)
            if not product_id:
        ...
        other_product_id = product_pool.search(cr, uid, [('default_code', '=', other_default_code)], context=context)
        ...
        _id = pool.get('product.link').create(cr, uid, {'product_id': product_id,
                                                        'linked_product_id': other_product_id,
                                                        'is_active': True,
                                                        'type': 'related',
                                                        }, context=context)
        if not _id:
